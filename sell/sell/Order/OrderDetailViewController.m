...
- (void)viewDidLoad {
     ...
     */
    if ([_dicOrder[@"state"] intValue] == 1) {
        [self setRightBtnShow];
        [self.btnRight setTitle:@"支付" forState:UIControlStateNormal];
    }
    else if ([_dicOrder[@"state"] intValue] == 2) {
        
    }
    else if ([_dicOrder[@"state"] intValue] == 3) {
        [self setRightBtnShow];
        [self.btnRight setTitle:@"确认收货" forState:UIControlStateNormal];
    }
    else if ([_dicOrder[@"state"] intValue] == 4) {
...
- (void)clickRightBtn:(UIButton *)sender
{
    ...
    if ([_dicOrder[@"state"] intValue] == 1) {
        postDic = @{@"command": @"changeOrder", @"state": @"2", @"orderId": _dicOrder[@"_id"]};
    }
    else if ([_dicOrder[@"state"] intValue] == 3) {
        postDic = @{@"command": @"changeOrder", @"state": @"4", @"orderId": _dicOrder[@"_id"]};
    }
    else {
        return;
    }
