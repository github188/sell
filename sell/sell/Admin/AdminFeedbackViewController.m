...
@interface AdminFeedbackViewController () <UITableViewDataSource, UITableViewDelegate>
...
@implementation AdminFeedbackViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    [self setControllerTitle:@"反馈列表"];
    _arrData = @[];
    WS(ws);
...
- (void)getDataFromNet
{
    NSDictionary *postDic = @{@"command": @"feedbackList"};
    ...
    [[HttpManager defaultManager] getRequestToUrl:HttpUrl params:postDic complete:^(BOOL successed, NSDictionary *result) {
        ...
        if (successed) {
            ...
            if (!self.arrData.count) {
                [[[UIAlertView alloc] initWithTitle:@"提示" message:@"暂时没有反馈" delegate:self cancelButtonTitle:@"知道了" otherButtonTitles:nil, nil] show];
            }
